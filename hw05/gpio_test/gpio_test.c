...
static unsigned int gpioLED1 = 50;       ///< hard coding the LED gpio for this example to P9_14 (GPIO50)
static unsigned int gpioLED2 = 14;       ///< hard coding the LED gpio for this example to P9_26 (GPIO14)
static unsigned int gpioButton1 = 117;   ///< hard coding the button gpio for this example to P9_25 (GPIO117)
static unsigned int gpioButton2 = 49;  ///< hard coding the button gpio for this example to P9_23 (GPIO49)
...
static bool     ledOn2 = 0;          ///< Is the LED on or off? Used to invert its state (off by default)
...
// gpio_set_value(gpioLED1, ledOn1);          // Not required as set by line above (here for reference)
   ...
   ledOn2 = true;
   gpio_request(gpioLED2, "sysfs");          // gpioLED1 is hardcoded to 49, request it
   gpio_direction_output(gpioLED2, ledOn2);   // Set the gpio to be in output mode and on
// gpio_set_value(gpioLED2, ledOn1);          // Not required as set by line above (here for reference)
   ...
   gpio_request(gpioButton2, "sysfs");       // Set up the gpioButton1
   ...
   result1 = request_irq(irqNumber1,             // The interrupt number requested
